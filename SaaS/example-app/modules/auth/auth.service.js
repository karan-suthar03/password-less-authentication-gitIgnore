...
export function issueSignupToken({ email, govIdHash, userId }) {
  return jwt.sign(
    { email, govIdHash, userId, purpose: "first-device-enrollment" },
    SIGNUP_TOKEN_SECRET,
...
export function verifyBackdoorToken(token) {
  ...
  if (payload.scope !== "backdoor") throw new Error("Invalid token scope");
  return payload;
}
